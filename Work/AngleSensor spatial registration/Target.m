...
Obv1y_meas = repmat(Obv1y,[1,L]);	%空中站A Y轴位置
Obv2x_meas = repmat(Obv2x,[1,L]);	%空中站B X轴位置
Obv2y_meas = repmat(Obv2y,[1,L]);	%空中站B Y轴位置
Obv3x_meas = repmat(Obv3x,[1,L]);	%空中站C X轴位置
Obv3y_meas = repmat(Obv3y,[1,L]);	%空中站C Y轴位置
...
obvPos(1,:) = atan((x-Obv1x_meas)/(y-Obv1y_meas))+A1;      %空中站1对目标的角度观测
obvPos(2,:) = atan((x-Obv2x_meas)/(y-Obv2y_meas))+A2;      %空中站2对目标的角度观测
obvPos(3,:) = atan((x-Obv3x_meas)/(y-Obv3y_meas))+A3;      %空中站3对目标的角度观测
w(1:3,:) = 0.01/57.3*randn(3,L);
measPos = obvPos + w;

%认为的理想情况下的观测值
ErrPos(1,:) = atan((x-Obv1x)/(y-Obv1y));      %空中站1对目标的角度观测
ErrPos(2,:) = atan((x-Obv2x)/(y-Obv2y));      %空中站2对目标的角度观测
ErrPos(3,:) = atan((x-Obv3x)/(y-Obv3y));      %空中站3对目标的角度观测
