function estimate_value = Kalman_CKF_10(model,measure_value,X_fun,Z_fun)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%程序名称: Kalman_CKF_A(model,measure_value,X_fun,Z_fun)
%程序说明:容积卡尔曼滤波方程
%参数说明:1、model  运动模型
%          model.Q 系统噪声协方差阵
%          model.R 观测噪声协方差阵
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for k=1:L
         ...
         [Z_pre,P_zz,P_xz] = z_time_update(Z_fun,X_pre,P_pre,model.R,measure_value.AngleErr{k},measure_value.SensorPos{k});      %时间更新-量测预测
...
%量测预测
function [Z_Pre,P_zz,P_xz] = z_time_update(Z_fun,X_pre,P_pre,R,AngleErr,SensorPos)
    ...
    Z_temp =Z_fun(x_temp,AngleErr,SensorPos); %计算传导容积点集
