...
%        3、X_fun 系统方程函数
%        4、Z_fun 量测方程函数
%        5、others 其他参数，根据需要更改
%输出说明：EstimateValue 滤波后的数据
%           EstimateValue.X 所有时刻的滤波结果，格式为1xL的矩阵胞组
%                          每个胞组中是Nx1维的量测数据
%           EstimateValue.P 所有时刻的误差协方差阵，格式为1xL的矩阵胞组
%                          每个胞组中是NxN维的量测数据（数据融合可能会用到）
%版本说明:v1.0 （2020-3-08 CRB 18235107312）建立文件
%       :v1.1 （2020-12-08 CRB）添加others参数，增加实用性
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for k=1:L
        ...
        others.k = k;
        [X_pre,P_pre] = x_time_update(X_fun,Est_temp,P_temp,model,other);      %时间更新-状态预测
...
function [X_Pre,P_Pre] = x_time_update(X_fun,x_est,P_temp,model,other)  
    ...
    X_temp =X_fun(x_CabutPointSet,model,other);              %计算传导容积点集
