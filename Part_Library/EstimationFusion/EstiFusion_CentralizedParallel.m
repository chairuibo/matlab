function [estimate_value ] = EstiFusion_CentralizedParallel(model,measure_value,model_count)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%程序说明   集中式融合系统，并行滤波程序
%参数说明   model  运动模型
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    est_x_update = cell(zlength+1,1);
    P_update = cell(zlength+1,1);
    ...
    for i = 1:count
        ...
        for j = 1:zlength
            measure_Z{j} =[ measure_value.HighFrequence.Z{j} measure_value.LowFrequence.Z{j}];   %行为每一次的量测数据，列为每个传感器的数据
